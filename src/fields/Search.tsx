...
export type SearchProps<T = string> = Omit<
...
export function Search<T = string>({
  ...
  const prevDefaultValuePropRef = useRef<T | undefined>(defaultValue)
  const [inputValue, setInputValue] = useState<string | undefined>(undefined)

  const [autoGeneratedOptions, setAutoGeneratedOptions] = useState<Option<T>[]>([])
  const [controlledDefaultValue, setDefaultControlledValue] = useState<T | undefined>(defaultValue)
  ...
  const fetchQueryResponse = useDebouncedCallback(
    async (_searched: string, _queryUrl: string, _queryMap: (record: Record<string, any>) => Option<T>) => {
      ...
      <Box ref={boxRef} isLight={isLight} onClick={open}>
        {boxRef.current && (
          <StyledAutoComplete
            ...
            open={isOpen}
            renderMenuItem={(itemLabel, item: ItemDataType<T>) =>
